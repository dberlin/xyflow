...
- [#5033](https://github.com/xyflow/xyflow/pull/5033) [`7b4a81fb`](https://github.com/xyflow/xyflow/commit/7b4a81fb6b3d88f8ee7b4f070aef7ac3b962d5a6) Thanks [@dimaMachina](https://github.com/dimaMachina)! - lint: use `Solid.JSX` type instead of the deprecated global `JSX` namespace
...
- [#5013](https://github.com/xyflow/xyflow/pull/5013) [`cde899c5`](https://github.com/xyflow/xyflow/commit/cde899c5be9715c4ff2cc331ea93821102604c62) Thanks [@moklick](https://github.com/moklick)! - Pass `NodeType` type argument from `SolidFlowProps` to `connectionLineComponent` property.
...
- [#4844](https://github.com/xyflow/xyflow/pull/4844) [`6f11e552`](https://github.com/xyflow/xyflow/commit/6f11e552c3f51b5931b98d5ff798156cdb8cc2d9) Thanks [@moklick](https://github.com/moklick)! - Allow custom data-testid for SolidFlow component
...
- [#4477](https://github.com/xyflow/xyflow/pull/4477) [`d5592e75`](https://github.com/xyflow/xyflow/commit/d5592e7508bc32d5ffc953844b1d42b9ec59b25b) Thanks [@peterkogo](https://github.com/peterkogo)! - Add `getNodesBounds` to `useSolidFlow`/`useSvelteFlow` hook as the new recommended way of determining node bounds.
...
- [#4574](https://github.com/xyflow/xyflow/pull/4574) [`b65aed19`](https://github.com/xyflow/xyflow/commit/b65aed19840c515949bef236a23d5f0a754cdeb4) Thanks [@bcakmakoglu](https://github.com/bcakmakoglu)! - Add `getHandleConnections` helper to `useSolidFlow` & `useSvelteFlow`
...
- [#4547](https://github.com/xyflow/xyflow/pull/4547) [`fdff601d`](https://github.com/xyflow/xyflow/commit/fdff601de418f2ac6a78f04e5a586d67b8d436e6) Thanks [@moklick](https://github.com/moklick)! - chore(solid): re-export Handle type
...
Solid Flow 12 is finally out! With a new package name `@xyflow/solid`!

### Main features

- **SSR / SSG**: you can define `width`, `height` and `handles` for the nodes. This makes it possible to render a flow on the server and hydrate on the client: [SSR guide](http://solidflow.dev/learn/advanced-use/ssr-ssg-configuration)
...
- **Solidive Flows:** The new hooks `useHandleConnections` and `useNodesData` and the new `updateNode` and `updateNodeData` functions can be used for managing the data flow between your nodes: [computing flows guide](http://solidflow.dev/learn/advanced-use/computing-flows)
  - Details: Working with solidive flows is super common. You update node A and want to solid on those changes in the connected node B. Until now everyone had to come up with a custom solution. With this version we want to change this and give you performant helpers to handle this. If you are excited about this, you can check out this example:
- **Dark mode and css variables:** Solid Flow now comes with a built-in dark mode, that can be toggled by using the new `colorMode` prop (”light”, “dark” or “system”): [dark mode example](https://solidflow.dev/examples/styling/dark-mode)
...
- **[`useConnection` hook](https://solidflow.dev/api-reference/hooks/use-connection):** With this hook you can access the ongoing connection. For example, you can use it for colorizing handles styling a custom connection line based on the current start / end handles.
- **Controlled `viewport`:** This is an advanced feature. Possible use cases are to animate the viewport or round the transform for lower res screens for example. This features brings two new props: [`viewport`](https://solidflow.dev/api-reference/solid-flow#viewport) and [`onViewportChange`](https://solidflow.dev/api-reference/solid-flow#on-viewport-change).
- **[`ViewportPortal`](https://solidflow.dev/api-reference/components/viewport-portal) component:** This makes it possible to render elements in the viewport without the need to implement a custom node.
- **[`onDelete`](https://solidflow.dev/api-reference/solid-flow#on-delete) handler**: We added a combined handler for `onDeleteNodes` and `onDeleteEdges` to make it easier to solid to deletions.
- **[`onBeforeDelete`](https://solidflow.dev/api-reference/solid-flow#on-before-delete) handler**: With this handler you can prevent/ manage deletions.
- **[`isValidConnection`](https://solidflow.dev/api-reference/solid-flow#is-valid-connection) prop:** This makes it possible to implement one validation function for all connections. It also gets called for programmatically added edges.
- **[`autoPanSpeed`](https://solidflow.dev/api-reference/solid-flow#autoPanSpeed) prop:** For controlling the speed while auto panning.
- **Background component**: add [`patternClass`](https://solidflow.dev/api-reference/components/background#pattern-class-name) prop to be able to style the background pattern by using a class name. This is useful if you want to style the background pattern with Tailwind for example.
...
- add `updateEdge` and `updateEdgeData` helpers to `useSolidFlow`
...
- cleanup `useSolidFlow`
- export `KeyCode` and `Align` type
- remove `Instance` in favour of `SolidFlowInstance` type
...
- fix `ref` prop for `SolidFlow` and `Handle` component
...
- add comments for types like `SolidFlowProps` or `Node` for a better developer experience
...
- batch `setNodes` and `setEdges` from `useSolidFlow`
...
Solid Flow v12 is coming soon! We worked hard over the past months and tried to make as few breaking changes as possible (there are some). We are in no rush to release v12, so we’d be happy to hear any early feedback so we can adjust the API or redefine new features before launching stable v12. 🚀 The big topics for this version are:

1. **Support for SSG/ SSR**: you can now render flows on the server
2. **Solidive flows**: new hooks and helper functions to simplify data flows
3. **Dark mode**: a new base style and easy way to switch between built in color modes

Svelte Flow had a big impact on this release as well. While combing through each line of Solid Flow, we created framework agnostic helpers, found bugs, and made some under the hood improvements. All of these changes are baked into the v12 release as a welcome side-effect of that launch. 🙌🏻 We also improved the performance for larger flows with the help of Ivan.

### Migrate from 11 to 12

Before you can try out the new features, you need to do some minor updates:

- **A new npm package name:** Our name changed from `solidflow` to `@xyflow/solid` and the main component is no longer a default, but a named import:
  - v11: `import SolidFlow from 'solidflow';`
  - v12: `import { SolidFlow } from '@xyflow/solid';`
...
- **SSR / SSG**: you can define `width`, `height` and `handles` for the nodes. This makes it possible to render a flow on the server and hydrate on the client: [codesandbox](https://codesandbox.io/p/devbox/solidflow-v12-next-pr66yh)
...
- **Solidive Flows:** The new hooks `useHandleConnections` and `useNodesData` and the new `updateNode` and `updateNodeData` functions can be used for managing the data flow between your nodes: [codesandbox](https://codesandbox.io/p/sandbox/solidflow-solidive-flow-sy93yx)
  - Details: Working with solidive flows is super common. You update node A and want to solid on those changes in the connected node B. Until now everyone had to come up with a custom solution. With this version we want to change this and give you performant helpers to handle this. If you are excited about this, you can check out this example:
- **Dark mode and css variables:** Solid Flow now comes with a built-in dark mode, that can be toggled by using the new `colorMode` prop (”light”, “dark” or “system”): [codesandbox](https://codesandbox.io/p/sandbox/solidflow-dark-mode-256l99)
  - Details: With this version we want to make it easier to switch between dark and light modes and give you a better starting point for dark flows. If you pass colorMode=”dark”, we add the class name “dark” to the wrapper and use it to adjust the styling. To make the implementation for this new feature easier on our ends, we switched to CSS variables for most of the styles. These variables can also be used in user land to customize a flow.
...
- **`onDelete` handler**: We added a combined handler for `onDeleteNodes` and `onDeleteEdges` to make it easier to solid to deletions.
...
- **Background component**: add `patternClass` to be able to style the background pattern by using a class name. This is useful if you want to style the background pattern with Tailwind for example.
- **`onMove` callback** gets triggered for library-invoked viewport updates (like fitView or zoom-in)
- **`deleteElements`** now returns deleted nodes and deleted edges
- add **`origin` attribute** for nodes
- add **`selectable` attribute** for edges
...
These changes are not really user-facing, but it could be important for folks who are working with the Solid Flow store:

- The biggest internal change is that we created a new package **@xyflow/system with framework agnostic helpers** that can be used be Solid Flow and Svelte Flow
...
**With v12 the `solidflow` package was renamed to `@xyflow/solid` - you can find the v11 source and the [`solidflow` changelog](https://github.com/xyflow/xyflow/blob/v11/packages/solidflow/CHANGELOG.md) on the v11 branch.**
