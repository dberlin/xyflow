...
export type NodeEventWithPointer<T = PointerEvent> = ({ node, event }: { node: Node; event: T }) => void;

export type NodesEventWithPointer<T = PointerEvent> = ({ nodes, event }: { nodes: Node[]; event: T }) => void;
