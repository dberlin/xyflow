...
export function useSolidFlow(): {
   ...
   */
  isNodeIntersecting: (nodeOrRect: Node | { id: Node['id'] } | Rect, area: Rect, partially?: boolean) => boolean;
  /**
   ...
   */
  updateNodeData: (id: string, dataUpdate: object | ((node: Node) => object), options?: { replace: boolean }) => void;
  /**
  ...
  function updateNode(
    ...
    store.nodes = /*untrack(() => store.nodes)*/ store.nodes.map((node) => {
      if (node.id === id) {
  ...
  function updateEdge(
    ...
    store.edges = /*untrack(() => store.edges)*/ store.edges.map((edge) => {
      if (edge.id === id) {
  ...
  return {
    ...
    // FIXME: This was $state.snapshot(store.viewport) in svelte, double check if this is correct
    getViewport: () => /*untrack(() => store.viewport)*/ store.viewport,
    ...
    isNodeIntersecting: (nodeOrRect: Node | { id: Node['id'] } | Rect, area: Rect, partially = true) => {
      const isRect = isRectObject(nodeOrRect);
      const nodeRect = isRect ? nodeOrRect : getNodeRect(nodeOrRect);
    ...
    deleteElements: async ({ nodes: nodesToRemove = [], edges: edgesToRemove = [] }) => {
      ...
      if (matchingNodes) {
        store.nodes = /*untrack(() => store.nodes)*/ store.nodes.filter(
          (node) => !matchingNodes.some(({ id }) => id === node.id)
        );
      }

      if (matchingEdges) {
        store.edges = /*untrack(() => store.edges)*/ store.edges.filter(
