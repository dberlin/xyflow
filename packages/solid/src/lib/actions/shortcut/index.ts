/** This is a small port of svelte-put/shortcut to solid-js as i work through converting shortcut handling to solid-js */
...
> {
  <Node extends Element>(
    ...args: undefined extends Parameter ? [node: Node, parameter?: Parameter] : [node: Node, parameter: Parameter]
...
function on(element: HTMLElement | Window, eventName: string, handler: EventListener) {
  element.addEventListener(eventName, handler);
  return () => element.removeEventListener(eventName, handler);
  // TODO: Check if we just want to use oncleanup
  // onCleanup(() => element.removeEventListener(eventName, handler));
}

/**
 * @param {ShortcutModifier} def
 * @returns {number}
 */
 ...
 * @param {ShortcutParameter} param - svelte action parameters
 * @returns {ShortcutActionReturn}
 */
export function shortcut(node: HTMLElement | Window, param: ShortcutParameter): ShortcutActionReturn {
  ...
  function handler(event: KeyboardEvent) {
    ...
    const modifierMask = [event.metaKey, event.altKey, event.shiftKey, event.ctrlKey].reduce((acc, value, index) => {
      if (value) {
        return acc | (1 << index);
      }
      return acc;
