...
export type SolidFlowJsonObject<NodeType extends Node = Node, EdgeType extends Edge = Edge> = {
  nodes: NodeType[];
  edges: EdgeType[];
  viewport: Viewport;
};
...
export type GeneralHelpers<NodeType extends Node = Node, EdgeType extends Edge = Edge> = {
   ...
   * @param partially - if true, the node is considered to be intersecting if it partially overlaps with the passed solid
   *
 ...
 * [`useSolidFlow`](/api-reference/hooks/use-solid-flow) hook or attaching a listener
 * to the [`onInit`](/api-reference/solid-flow#event-oninit) event.
 *
 * @public
 */
export type SolidFlowInstance<NodeType extends Node = Node, EdgeType extends Edge = Edge> = GeneralHelpers<
  NodeType,
  EdgeType
> &
