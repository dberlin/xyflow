...
import { InternalNode, type Node } from '../types';
...
export function useMoveSelectedNodes() {
  ...
  const moveSelectedNodes = (params: { direction: XYPosition; factor: number }) => {
    const { nodeExtent, snapToGrid, snapGrid, nodesDraggable, onError, nodeLookup, nodeOrigin } = store.getState();
    const { updateNodePositions } = store.getActions();
