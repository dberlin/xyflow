...
import type { InternalNode, Node, SolidFlowStore } from '../types';

function storeSelector(s: SolidFlowStore) {
...
): (s: SolidFlowStore) => SelectorReturn | ConnectionState<InternalNode> {
  if (connectionSelector) {
    const combinedSelector = (s: SolidFlowStore) => {
 ...
 *
 * ```tsx
 *import { useConnection } from '@xyflow/solid';
 *
 *function App() {
